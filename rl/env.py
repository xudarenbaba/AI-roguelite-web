...
OBSTACLE_LAYOUTS: list[list[dict]] = [
    # 布局 0：三角分布（中央偏左方块 + 右上方块 + 右下方块）
    [
        {"x": 370, "y": 215, "w": 90, "h": 90},
        {"x": 580, "y": 130, "w": 80, "h": 80},
        {"x": 580, "y": 330, "w": 80, "h": 80},
    ],
    # 布局 1：横向三列（左中右均匀分布）
    [
        {"x": 260, "y": 210, "w": 85, "h": 85},
        {"x": 430, "y": 210, "w": 85, "h": 85},
        {"x": 600, "y": 210, "w": 85, "h": 85},
    ],
    # 布局 2：对角线分布（左上 + 中央 + 右下）
    [
        {"x": 260, "y": 140, "w": 90, "h": 90},
        {"x": 415, "y": 230, "w": 80, "h": 80},
        {"x": 600, "y": 320, "w": 90, "h": 90},
    ],
    # 布局 3：品字形（上方一块 + 下方两块）
    [
        {"x": 415, "y": 150, "w": 90, "h": 90},
        {"x": 290, "y": 310, "w": 85, "h": 85},
        {"x": 560, "y": 310, "w": 85, "h": 85},
    ],
]
...
_SEG5 = N_RAYS                   # 16（射线检测，与障碍物数量无关，不需要改）
_SEG6 = 4
_SEG7 = N_ACTIONS                # 9
_SEG8 = 1
_SEG9 = 1                        # 最危险子弹 TTA
...
class AssaultEnv(gym.Env):
        ...
        if target is not None:
            ...
            if dist_now < ASSAULT_KITE_RANGE:
                ...
                if los:
                    ...
                    reward -= 0.006   # LOS 遮挡惩罚恢复旧版本权重（-0.015 过重会惩罚正常绕路）
            elif ASSAULT_ATTACK_RANGE <= dist_now < 130.0:
                if los:
                    reward += 0.006
                else:
                    reward -= 0.003   # 稍远遮挡轻微惩罚
    ...
    def _curriculum_obstacles(self) -> list[dict]:
        ...
        if stage == 0:
            return []   # 无障碍
        if stage == 1:
            # 单块方形障碍（中央正方形），与新布局风格一致
            return [{"x": 405, "y": 225, "w": 90, "h": 90}]
        if stage == 2:
